...
function syncVoucherAmountFromLimits(normalizeValue = false) {
    if (!voucherAmount) {
        return 0;
    }
    const total = parseFloat(totalPrice.value || 0) || 0;
    const balance = getSelectedClientVoucherBalance();
    const maxAllowed = Math.max(0, Math.min(total, balance));

    let requested = parseFloat(voucherAmount.value || 0);
    if (!isFinite(requested) || requested < 0) {
        requested = 0;
    }

    voucherAmount.max = maxAllowed.toFixed(2);
    const applied = Math.min(requested, maxAllowed);

    if (normalizeValue) {
        voucherAmount.value = applied > 0 ? applied.toFixed(2) : '';
    }
    return applied;
}
...
function updateVoucherPreview() {
    const total = parseFloat(totalPrice.value || 0) || 0;
    const balance = getSelectedClientVoucherBalance();
    const applied = syncVoucherAmountFromLimits(false);
...
if (voucherAmount) {
    ...
    voucherAmount.addEventListener('blur', function () {
        syncVoucherAmountFromLimits(true);
        updateVoucherPreview();
    });
}
