-- Test Attendance Data for Hourly Staff (Clean Version)
-- This script DELETES existing test data first, then adds fresh attendance records
...
-- CLEAN UP: Delete existing test attendance data for this user in the billing period
DELETE ab FROM attendance_breaks ab
INNER JOIN staff_attendance sa ON sa.id = ab.attendance_id
...
SELECT 'Cleaned up existing test data' AS status;

-- Now add fresh attendance records for the current billing period (March 16 - April 15, 2026)
