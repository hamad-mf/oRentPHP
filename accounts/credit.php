...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_payment') {
        ...
        try {
            ...
            $creditIncomeNow = (float) $pdo->query("SELECT COALESCE(SUM(amount),0) FROM ledger_entries WHERE payment_mode='credit' AND txn_type='income'")->fetchColumn();
            $creditExpenseNow = (float) $pdo->query("SELECT COALESCE(SUM(amount),0) FROM ledger_entries WHERE payment_mode='credit' AND txn_type='expense'")->fetchColumn();
                ...
                <tbody class="divide-y divide-mb-subtle/10">
                    ...
                    <?php else:
                        foreach ($entries as $row):
                            ...
                            $canRowSettle = $canAddPayment && $isIncome && $prefillAmount > 0;
                    ...
                    ?>
                        <tr class="hover:bg-mb-black/30 transition-colors">
                            ...
                            <td class="px-6 py-3 text-right <?= $amtColor ?> font-medium whitespace-nowrap"><?= $isIncome ? '+' : '-' ?>$<?= number_format($rowAmount, 2) ?></td>
