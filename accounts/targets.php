...
$aS = $pdo->prepare("SELECT DATE(posted_at) AS day, SUM(amount) AS achieved
    FROM ledger_entries WHERE txn_type='income'
    AND DATE(posted_at)>=? AND DATE(posted_at)<=? GROUP BY DATE(posted_at)");
...
$pct  = $monthlyTarget > 0 ? min(100, round($totalAchieved / $monthlyTarget * 100, 1)) : 0;
$curY = (int)date('Y');
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    ...
    <div class="bg-mb-surface border border-<?= $totalAchieved>0?'green-500/20':'mb-subtle/20' ?> rounded-xl p-5">
        <p class="text-xs text-mb-subtle uppercase tracking-wider mb-2">Total Achieved</p>
        <p class="text-2xl font-light <?= $totalAchieved>0?'text-green-400':'text-mb-subtle' ?>">$<?= number_format($totalAchieved,2) ?></p>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-mb-subtle/10">
                ...
                <tr class="hover:bg-mb-black/30 transition-colors <?= $row['is_today']?'border-l-2 border-mb-accent':'' ?>">
                    ...
                    <td class="px-6 py-3 text-right font-medium <?= $row['achieved']>0?'text-white':'text-mb-subtle' ?>"><?= $row['is_future']?'—':'$'.number_format($row['achieved'],2) ?></td>
