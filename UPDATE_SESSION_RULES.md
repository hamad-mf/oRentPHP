...
| 2026-03-21 | attendance_admin_controls | Yes | `migrations/releases/2026-03-21_attendance_admin_controls.sql` | Pending | Admin can now force punch-out active staff sessions (auto-closes open breaks) and edit any attendance record (times, late/early reasons). Adds admin_note + is_manual_punch to staff_attendance. |
| 2026-03-21 | period_labels_15to15 | Yes | None | N/A | Updated month dropdowns on Payroll (list filter + generate modal), Staff Advances, Staff Incentives, and Targets screen to show 15-to-15 period labels (e.g. "15 Mar – 15 Apr") instead of plain month names. |
| 2026-03-21 | emi_list_overdue_sort | Yes | None | N/A | EMI list now defaults to overdue-first sort: overdue EMIs (nearest date first) → due today → upcoming (nearest first) → completed last. |
| 2026-03-25 | deposit_extension_usage | Yes | `migrations/releases/2026-03-25_deposit_extension_usage.sql` | Pending | Extension payment from deposit: pay for extensions using security deposit with split payment support. Adds deposit tracking columns to reservations and reservation_extensions. |
| 2026-03-25 | vehicle_sold_status | Yes | `migrations/releases/2026-03-25_vehicle_sold_status.sql` | Pending | Sold lifecycle status for vehicles: extends vehicles.status ENUM, adds sold_at column, guards all operational screens, adds Sold card to fleet list. |
| 2026-03-27 | vehicle_monthly_targets | Yes | `migrations/releases/2026-03-27_vehicle_monthly_targets.sql` | Pending | Vehicle monthly income targets: dedicated screen for setting and tracking monthly income targets per vehicle with bulk operations, visual achievement indicators, and Hope Window integration. |
