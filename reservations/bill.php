...
// ── Deposit vars (including new resolution columns) ───────────────────────────
...
<head>
    ...
    <title>Invoice #<?= $id ?> — O Rent CRM</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .action-bar .back { color: #aaa; text-decoration: none; font-size: 13px; }
        .action-bar .back:hover { color: #fff; }
        .action-bar .title { color: #fff; font-size: 14px; font-weight: 500; }
        ...
        .invoice-wrap {
            ...
            border-radius: 12px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,.12);
        }
        .inv-header {
            background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%);
            padding: 36px 40px; display: flex; justify-content: space-between; align-items: flex-start;
        }
        .company-name { font-size: 28px; font-weight: 700; color: #fff; letter-spacing: -0.5px; }
        .company-sub  { color: #64748b; font-size: 13px; margin-top: 4px; }
        .inv-badge    { text-align: right; }
        .inv-badge .label { color: #64748b; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; }
        .inv-badge .num   { font-size: 32px; font-weight: 300; color: #38bdf8; line-height: 1; }
        .inv-badge .status-pill {
            display: inline-block; margin-top: 8px; padding: 4px 12px; border-radius: 50px;
        ...
        .inv-meta {
            background: #f8fafc; padding: 20px 40px; display: flex; gap: 40px;
            border-bottom: 1px solid #e2e8f0; font-size: 13px;
        }
        .inv-meta .meta-item .label { color: #94a3b8; text-transform: uppercase; font-size: 10px; letter-spacing: .8px; margin-bottom: 3px; }
        .inv-meta .meta-item .val   { color: #1e293b; font-weight: 500; }
        ...
        .info-card h4 { font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #94a3b8; margin-bottom: 12px; }
        .info-card .name   { font-size: 16px; font-weight: 600; color: #0f172a; margin-bottom: 6px; }
        .info-card .detail { font-size: 13px; color: #475569; line-height: 1.7; }

        .period-row {
            background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 8px;
            padding: 16px 20px; display: flex; align-items: center; gap: 20px; margin-bottom: 28px;
        }
        .period-row .period-item .label { font-size: 10px; text-transform: uppercase; letter-spacing: .8px; color: #60a5fa; margin-bottom: 3px; }
        .period-row .period-item .val   { font-size: 14px; font-weight: 600; color: #1e40af; }
        .period-row .arrow      { color: #60a5fa; font-size: 20px; }
        .period-row .days-badge { margin-left: auto; background: #3b82f6; color: #fff; padding: 6px 18px; border-radius: 50px; font-size: 13px; font-weight: 600; }

        .inspection-row { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 28px; }
        .insp-card { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px 20px; }
        .insp-card h4  { font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #94a3b8; margin-bottom: 10px; }
        .insp-card .row { display: flex; justify-content: space-between; font-size: 13px; color: #475569; padding: 3px 0; }
        .insp-card .row strong { color: #1e293b; }

        .price-table { width: 100%; border-collapse: collapse; margin-bottom: 8px; }
        .price-table th { background: #f1f5f9; text-align: left; padding: 10px 16px; font-size: 11px; text-transform: uppercase; letter-spacing: .8px; color: #64748b; }
        .price-table td { padding: 12px 16px; font-size: 14px; border-bottom: 1px solid #f1f5f9; }
        .price-table .val { text-align: right; font-weight: 500; }
        .total-row td  { font-weight: 700; font-size: 18px; color: #0f172a; border-top: 2px solid #e2e8f0; }
        .total-row .val { color: #059669; }
        .overdue-row td { color: #dc2626; }

        .inv-footer {
            background: #f8fafc; border-top: 1px solid #e2e8f0; padding: 24px 40px;
            display: flex; justify-content: space-between; align-items: center; font-size: 12px; color: #94a3b8;
        }
        .inv-footer .generated { font-style: italic; }

        @media print {
            .action-bar { display: none !important; }
            body { background: #fff; }
            .invoice-wrap { box-shadow: none; border-radius: 0; margin: 0; max-width: 100%; }
            .inv-header, .inv-badge .status-pill, .period-row, .info-card, .insp-card, .price-table th {
    ...
    <div class="invoice-wrap">
        ...
        <div class="inv-header">
            ...
            <div class="inv-badge">
                ...
                $cls = $sc[$r['status']] ?? '';
                ?>
                <span class="status-pill <?= $cls ?>"><?= ucfirst($r['status']) ?></span>
            </div>
        </div>

        <!-- Meta -->
        <div class="inv-meta">
            <div class="meta-item"><div class="label">Issue Date</div><div class="val"><?= date('d M Y') ?></div></div>
            <div class="meta-item"><div class="label">Rental Type</div><div class="val"><?= ucfirst($r['rental_type']) ?></div></div>
            <div class="meta-item"><div class="label">Duration</div><div class="val"><?= $days ?> Day<?= $days > 1 ? 's' : '' ?></div></div>
            <div class="meta-item"><div class="label">Reserved On</div><div class="val"><?= date('d M Y', strtotime($r['created_at'])) ?></div></div>
        </div>
        ...
        <div class="inv-body">
            ...
            <div class="two-col">
                <div class="info-card">
                    ...
                    <div class="detail">
                        <?php if ($r['client_phone']): ?>📞 <?= e($r['client_phone']) ?><br><?php endif; ?>
                        <?php if ($r['client_email']): ?>✉ <?= e($r['client_email']) ?><br><?php endif; ?>
                        <?php if ($r['client_address']): ?>📍 <?= e($r['client_address']) ?><?php endif; ?>
                    </div>
                </div>
                <div class="info-card">
                    <h4>Vehicle</h4>
                    <div class="name"><?= e($r['brand']) ?> <?= e($r['model']) ?></div>
                    <div class="detail">
            ...
            <div class="period-row">
                <div class="period-item"><div class="label">Start</div><div class="val"><?= fdt($r['start_date']) ?></div></div>
                <div class="arrow">→</div>
                <div class="period-item"><div class="label">End</div><div class="val"><?= fdt($r['end_date']) ?></div></div>
                ...
                <div class="inspection-row">
                    <?php if ($delivery): ?>
                        <div class="insp-card">
                            ...
                            <div class="row"><span>Mileage</span><strong><?= number_format($delivery['mileage']) ?> km</strong></div>
                            <div class="row"><span>Fuel Level</span><strong><?= $delivery['fuel_level'] ?>%</strong></div>
                            <div class="row"><span>Date</span><strong><?= fdt($delivery['created_at']) ?></strong></div>
                            <?php if ($delivery['notes']): ?>
                                <div class="row" style="flex-direction:column;gap:2px"><span>Notes</span><span style="color:#475569;font-size:12px"><?= e($delivery['notes']) ?></span></div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                    <?php if ($return): ?>
                        <div class="insp-card">
                            ...
                            <div class="row"><span>Mileage</span><strong><?= number_format($return['mileage']) ?> km</strong></div>
                            <div class="row"><span>Fuel Level</span><strong><?= $return['fuel_level'] ?>%</strong></div>
                            <div class="row"><span>Date</span><strong><?= fdt($return['created_at']) ?></strong></div>
                            <?php if ($return['notes']): ?>
                                <div class="row" style="flex-direction:column;gap:2px"><span>Notes</span><span style="color:#475569;font-size:12px"><?= e($return['notes']) ?></span></div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                </div>
            ...
            <table class="price-table">
                ...
                <tbody>
                    ...
                    <?php if ($advancePaid > 0): ?>
                        <tr style="color:#7c3aed"><td>Advance Collected</td><td class="val">-$<?= number_format($advancePaid, 2) ?></td></tr>
                    <?php endif; ?>
                    <?php if ($deliveryPrepaid > 0): ?>
                        <tr style="color:#0284c7"><td>Delivery Charge Collected at Booking</td><td class="val">+$<?= number_format($deliveryPrepaid, 2) ?></td></tr>
                    <?php endif; ?>
                    <?php if ($extensionPaid > 0): ?>
                        <tr style="color:#0ea5e9"><td>Extension Collected (Grace)</td><td class="val">+$<?= number_format($extensionPaid, 2) ?></td></tr>
                    <?php endif; ?>
                    <?php if ($deliveryCharge > 0): ?>
                        <tr style="color:#0369a1"><td>Delivery Charge</td><td class="val">+$<?= number_format($deliveryCharge, 2) ?></td></tr>
                    <?php endif; ?>
                    <?php if ($delivDiscountAmt > 0): ?>
                        <tr style="color:#16a34a"><td>🎫 Delivery Discount<?= $delivDiscType === 'percent' ? ' (' . $delivDiscVal . '%)' : '' ?></td><td class="val">-$<?= number_format($delivDiscountAmt, 2) ?></td></tr>
                    <?php endif; ?>
                    ...
                    <?php if (!$isQuotation): ?>
                        <?php if ($overdueAmt > 0): ?>
                            <tr class="overdue-row"><td>⚠ Overdue / Late Charges</td><td class="val">+$<?= number_format($overdueAmt, 2) ?></td></tr>
                        <?php endif; ?>
                        <?php if ($kmOverageChg > 0): ?>
                            <tr style="color:#d97706"><td>🚗 KM Overage (<?= number_format($r['km_driven']) ?> km driven, limit <?= number_format($r['km_limit']) ?> km)</td><td class="val">+$<?= number_format($kmOverageChg, 2) ?></td></tr>
                        <?php endif; ?>
                        <?php if ($damageChg > 0): ?>
                            <tr style="color:#ea580c"><td>🔧 Damage Charges</td><td class="val">+$<?= number_format($damageChg, 2) ?></td></tr>
                        <?php endif; ?>
                        <?php if ($additionalChg > 0): ?>
                            <tr style="color:#c2410c"><td>Return Pickup Charge</td><td class="val">+$<?= number_format($additionalChg, 2) ?></td></tr>
                        <?php endif; ?>
                        <?php if ($chellanChg > 0): ?>
                            <tr style="color:#dc2626"><td>🚔 Chellan</td><td class="val">+$<?= number_format($chellanChg, 2) ?></td></tr>
                        <?php endif; ?>
                        <?php if ($discountAmt > 0): ?>
                            <tr style="color:#16a34a"><td>🎫 Return Discount<?= $discType === 'percent' ? ' (' . $discVal . '%)' : '' ?></td><td class="val">-$<?= number_format($discountAmt, 2) ?></td></tr>
                        <?php endif; ?>
                        <?php if ($returnVoucherApplied > 0): ?>
                            <tr style="color:#10b981"><td>Voucher Applied on Return</td><td class="val">-$<?= number_format($returnVoucherApplied, 2) ?></td></tr>
                        <?php endif; ?>
                        <?php if ($earlyVoucherCredit > 0): ?>
                            <tr style="color:#10b981"><td>Early Return Voucher Credit (for next booking)</td><td class="val">+$<?= number_format($earlyVoucherCredit, 2) ?></td></tr>
                        <?php endif; ?>
                    <?php endif; ?>
                </tbody>
                <tfoot>
                    ...
                    <?php else: ?>
                        ...
                        <?php if ($depAmount > 0): ?>
                            <tr style="background:#f0fdf4;color:#166534">
                                <td style="padding:10px 16px;font-size:13px"><strong>Security Deposit:</strong> Collected</td>
                            ...
                            <?php endif; ?>
                            <?php if ($depHeld > 0): ?>
                                <?php if ($depHeldAction === 'released'): ?>
                                    <tr style="color:#16a34a">
                                        <td style="padding:8px 16px;font-size:12px">
                                            &#x2713; Held Amount Released to Client
                                            <?= $depHeldResolvedAt ? ' <span style="color:#94a3b8;font-size:11px">(' . date('d M Y', strtotime($depHeldResolvedAt)) . ')</span>' : '' ?>
                                        </td>
                                        <td class="val" style="padding:8px 16px;font-size:12px">-$<?= number_format($depHeld, 2) ?></td>
                                    </tr>
                                <?php elseif ($depHeldAction === 'converted'): ?>
                                    <tr style="color:#dc2626">
                                        <td style="padding:8px 16px;font-size:12px">
                                            &#x2713; Held Amount Converted to Income
                                            <?= $depHeldResolvedAt ? ' <span style="color:#94a3b8;font-size:11px">(' . date('d M Y', strtotime($depHeldResolvedAt)) . ')</span>' : '' ?>
                                        </td>
                                        <td class="val" style="padding:8px 16px;font-size:12px">-$<?= number_format($depHeld, 2) ?></td>
                                    </tr>
                                <?php else: ?>
                                    <tr style="color:#ca8a04">
                                        <td style="padding:8px 16px;font-size:12px">
                                            ...
                                            <span style="color:#94a3b8;font-size:11px">(Pending resolution)</span>
                                        </td>
                                        <td class="val" style="padding:8px 16px;font-size:12px">-$<?= number_format($depHeld, 2) ?></td>
                                    </tr>
                                <?php endif; ?>
        ...
        <div class="inv-footer">
            ...
            <span class="generated">Generated: <?= date('d M Y, h:i A') ?></span>
        </div>
