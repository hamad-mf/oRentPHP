...
// Use POST values on validation error, otherwise DB values
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3 text-sm text-mb-subtle">
        ...
        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
        <span class="text-white">Edit Challan</span>
    </div>

    <h2 class="text-white text-2xl font-light">Edit Challan</h2>
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="bg-mb-surface border border-mb-subtle/20 rounded-xl p-6 space-y-5">
            ...
            <!-- Paid challan warning -->
            <?php if ($challan['status'] === 'paid'): ?>
                <div class="flex items-center gap-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg px-4 py-3 text-xs text-yellow-400">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                    This challan has already been marked as paid. Editing will not affect the existing ledger entry.
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="flex items-center justify-end gap-4">
            ...
            <button type="submit"
                class="bg-mb-accent text-white px-8 py-3 rounded-full hover:bg-mb-accent/80 transition-colors text-sm font-medium shadow-lg shadow-mb-accent/20">Update Challan</button>
        </div>
    </form>
</div>
