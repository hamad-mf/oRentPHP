...
| 2026-04-05 | vehicle_permanent_scratches | `migrations/releases/2026-04-05_vehicle_permanent_scratches.sql` | Creates `vehicle_permanent_scratches` table for storing permanent scratch/damage photos and descriptions per vehicle. These scratches persist across all reservations and auto-populate in delivery/return inspection interfaces. Run before deploying vehicle-permanent-scratches feature. |
| 2026-04-09 | user_session_tracking | `migrations/releases/2026-04-09_user_session_tracking.sql` | Adds `is_online`, `last_login_at`, `last_logout_at`, `session_id` columns to `users` table for real-time online/offline status tracking. Run before deploying real-time staff monitor status feature. |
| 2026-04-15 | reservation_additional_info | `migrations/releases/2026-04-15_reservation_additional_info.sql` | Adds `delivery_location` VARCHAR(255), `return_location` VARCHAR(255), and `additional_note` TEXT columns to `reservations` table for storing supplementary reference information. Run before deploying reservation-additional-info feature. |
